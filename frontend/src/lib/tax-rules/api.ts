...
function mapToPayload(rule: Partial<TaxRule>): Record<string, unknown> {
  return {
    ...
    tags: rule.tags,
    condition_summary: (rule.condition as Record<string, unknown>)?.summary ?? "",
    result_type: (rule.result as Record<string, unknown>)?.type ?? "amount",
    result_value: (rule.result as Record<string, unknown>)?.value ?? null,
    result_unit: (rule.result as Record<string, unknown>)?.unit ?? "",
    result_formula: (rule.result as Record<string, unknown>)?.formula ?? "",
    notes: (rule.result as Record<string, unknown>)?.notes ?? "",
